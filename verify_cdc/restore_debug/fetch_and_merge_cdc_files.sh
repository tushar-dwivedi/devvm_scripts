fetch_cdc_files_from_all_nodes_and_merge() {
  ...
  cat $filtered_for_table_json_file | jq -s 'add' | jq -s 'map(. + identify_db_operations)' | jq 'highlight_operations' >$final_grouped_file
  # cat $result_dir_local/node_filtered_${table_name}_* | jq -s 'add' | jq -s 'map(. + identify_db_operations)' | jq 'highlight_operations' >$final_grouped_file

  cat $filtered_for_table_json_file | jq -s 'add' | jq -s 'map(. + identify_db_operations)' | jq 'find_underreplicated_events' >${result_dir_local}/redundancy_count.json
  # cat $result_dir_local/node_filtered_${table_name}_* | jq -s 'add' | jq -s 'map(. + identify_db_operations)' | jq 'find_underreplicated_events' >${result_dir_local}/redundancy_count.json

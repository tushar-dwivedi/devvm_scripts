...
python3 -m jedi.tools.sdt_runner --test_target //jedi/e2e/callisto:crdb_load_test --bodega_sid ${bodega_order_id} -- -k "test_custom_perf" --crdb_populate_rows 100000 --crdb_load_duration 60m --crdb_load_name files --crdb_load_type cqlproxy
...
ssh -i $pem_file ubuntu@$first_node_ip "bash -s" <./skip_commit/verify_cdc/restore_run/dump_kafka_data.sh
if [ $? -eq 0 ]; then
...
bash ./skip_commit/verify_cdc/restore_debug/start_restore_debug.sh "files_perf_test_only" sd sd_restore 0
bash ./skip_commit/verify_cdc/restore_debug/start_restore_debug.sh "files_perf_test_only" sd_restore sd_restore_kafka 1
# echo -e '\a'
